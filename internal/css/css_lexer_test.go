...
// (skip void, comments and don't care content of End type Token)
func testTokens(expecteds []Token, lexer *Lexer) error {
	i := 0
	for {
		actual := lexer.getNextToken()
		if actual.Type == Void || actual.Type == Comment {
			continue
		}
		if i >= len(expecteds) {
		...
		} // else if actual.Type == End {
		// 	return fmt.Errorf("Sequence end before expected, expected: %v", expected)
		// }
...
input[type="text"] {
  border: 1px solid #ccc;
		...
		{
			name: "with comments",
			raw:  `/*hello*/ h1 {color: /*hello*/ red; /*hello*/ font-size: 14px;}`,
			expected: []Token{
				{Type: Selector, Content: "h1"},
				{Type: Property, Content: "color"},
				{Type: Value, Content: "red"},
				{Type: Property, Content: "font-size"},
				{Type: Value, Content: "14px"},
				{Type: End, Content: ""},
			},
